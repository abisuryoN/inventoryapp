...
private boolean modeEdit = false;
    public TambahKategori(java.awt.Frame parent, boolean modal) {
        ...
        initComponents();
         ...
         // Pastikan itemnya diset dulu (jika belum di-set lewat Properties)
    // jComboBox_Status.removeAllItems();
    // jComboBox_Status.addItem("Aktif");
    // jComboBox_Status.addItem("Tidak Aktif");

    // --- KODE UNTUK MEWARNAI COMBO BOX ---
    JComboBox_kategori.setRenderer(new DefaultListCellRenderer() {
        @Override
        public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
            Component c = super.getListCellRendererComponent(list, value, index, isSelected, cellHasFocus);
            
            // Mengecek nilai teks pada pilihan
            if (value != null) {
                String status = value.toString();
                if (status.equalsIgnoreCase("Aktif")) {
                    c.setForeground(new Color(0, 153, 0)); // Warna Hijau
                } else if (status.equalsIgnoreCase("Tidak Aktif")) {
                    c.setForeground(Color.RED); // Warna Merah
                } else {
                    c.setForeground(list.getForeground()); // Warna Default Hitam
                }
            }
            
            // Menjaga agar saat kursor diarahkan (hover/selected), teks tetap putih agar terbaca
            if (isSelected) {
                c.setForeground(Color.WHITE);
                c.setBackground(new Color(0, 120, 215)); // Warna background biru default seleksi Windows
            } else {
